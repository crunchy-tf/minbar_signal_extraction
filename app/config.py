...
from pydantic import Field, AnyHttpUrl # Added AnyHttpUrl
...
class Settings(BaseSettings):
    ...
    SIGNAL_EXTRACTION_STATUS_FIELD_IN_SOURCE: str = Field(default="signal_extraction_status") # Field in SOURCE table
    ...
    # --- Keyword Manager (Optional, if needed for topic_name enrichment) ---
    ...
    def target_timescaledb_dsn_asyncpg(self) -> str:
        # TimescaleDB uses the same DSN format as PostgreSQL
...
# Make global config vars available if your other modules still use them (prefer settings object)
